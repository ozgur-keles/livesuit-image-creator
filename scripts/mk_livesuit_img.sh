...
do_pack_a20()
{
    echo "Packing for linux for A20"
    ...
    # update u-boot.bin
    if [ -f ${IN_DIR}/u-boot.bin ]; then
        cp ${IN_DIR}/u-boot.bin bootfs/linux/ -f
    fi
    # update uImage
    if [ -f ${IN_DIR}/uImage ]; then
        cp ${IN_DIR}/uImage bootfs/ -f
    fi
    # update uEnv.txt
    if [ -f ${IN_DIR}/uEnv.txt ]; then
        cp ${IN_DIR}/uEnv.txt bootfs/ -f
    fi
